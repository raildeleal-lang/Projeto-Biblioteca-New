...
# --- FORMULÁRIO (DEVE FICAR FORA DA FUNÇÃO) ---
class CadastroUsuarioForm(forms.Form):
    estilo_campo = 'padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; width: 100%; font-size: 0.95rem;'

    nome = forms.CharField(max_length=50,
                           widget=forms.TextInput(attrs={'style': estilo_campo, 'placeholder': 'Primeiro Nome'}))
    sobrenome = forms.CharField(max_length=50,
                                widget=forms.TextInput(attrs={'style': estilo_campo, 'placeholder': 'Sobrenome'}))
    username = forms.CharField(max_length=50, widget=forms.TextInput(attrs={
        'style': 'padding: 10px; border-radius: 0 6px 6px 0; border: 1px solid #cbd5e1; width: 100%; font-size: 0.95rem;',
        'placeholder': 'Ex: carlos.silva'}))
    password = forms.CharField(required=False, widget=forms.PasswordInput(
        attrs={'style': estilo_campo, 'placeholder': 'Digite a senha'}))
    matricula = forms.CharField(max_length=20,
                                widget=forms.TextInput(attrs={'style': estilo_campo, 'placeholder': 'Ex: 2026123'}))
    tipo = forms.ChoiceField(choices=PerfilUsuario.TIPO_CHOICES, widget=forms.Select(attrs={
        'style': 'padding: 10px; border-radius: 6px; border: 1px solid #cbd5e1; width: 100%; font-size: 0.95rem; background-color: #fff;'}))
